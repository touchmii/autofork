...
class f124():
    def __init__(self, net, node, eds):
        ...
        self.node.tpdo.read()
        self.node.rpdo.read()
        self.node.tpdo[self.x].clear()
        self.node.tpdo[self.x].add_variable('Position_demand_value')
        self.node.tpdo[self.x].add_callback(self.pdo_callback)
        self.node.trans_type = 254
        self.node.event_timer = 100
        self.node.enable = True

        self.node.rpdo[self.x].clear()
        self.node.rpdo[self.x].add_variable('Target_velocity')
        self.node.rpdo[self.x].enable = True

        self.node.nmt.state = 'PRE-OPERATIONAL'
        self.node.tpdo.save()
#        self.node.rpdo.save()
#        self.node.rpdo[self.x]['Target_velocity'].raw = 0
#        self.node.rpdo[self.x].start(0.1)

        self.node.nmt.state = 'OPERATIONAL'

    def pdo_callback(self, message):
        for var in message:
            print('receve id',self.node_idd, ' xx ', var.name, var.raw)
    ...
    def set_motor_speed_pdo(self, v):
        self.node.rpdo[self.x]['Target_velocity'].raw = v*self.rpm_scale

async def motion_control(q):
    network = canopen.Network()
    network.connect(channel='can0', bustype='socketcan')
#    network.connect(channel=0 , bustype='canalystii', bitrate=500000)
    motor1 = f124(network, 2, 'KINCO-JDFD.EDS')
    time.sleep(2)
...
#    motor2.set_motor_control('reset')
    ...
    while True:
        ...
        motor1.set_motor_speed_pdo(command[0])
        motor2.set_motor_speed_pdo(command[1])
