...
async def read_event(dev, queue):
    async for ev in dev.async_read_loop():
        #print(ev.type == 3)
        left_x_p, left_y_p = 0, 0
        if( (ev.type == 3 and ev.code < 6) or ev.type == 1):
            #print(repr(ev))
...
def detect_event(event):
    joystick_dict = {1:'LEFT_Y', 0:'LEFT_X', 2:'RIGHT_X', 3:'L2_Analog', 4:'R2_Analog', 5:'RIGHT_Y', 0x16:'UPDOWN', 0x17:'LEFTRIGHT'}
    ...
    e, v = '', None
    if(event[0] == 3 and event[1] < 6):
        e = joystick_dict[event[1]]
        v = event[2]
    elif(event[0] == 1 and event[1] < 5):
