...
class f124():
    def __init__(self, net, node, eds, rpm_s=5120000/1875):
        self.rpm_scale = rpm_s
        ...
        self.motor_control_dict = {'enable_speed':0x0F, 'enable_localtion':0x2F, 'disable':0x06, 'reset': 0x86, 'enable_home': 0x1F}
...
#    motor2.set_motor_control('reset')
    ...
    motor1.node.sdo['Profile_deceleration'].raw = 163.84*20
    ...
    while True:
        command = await q.get()
        print('driver_motor_speed: {}, steering_motor_speed: {}'.format(command[0],command[1]))
...
async def joystick_control(event_queue, command_q):
    ...
    while True:
        ...
        if(event[0] == 3):
            ...
            command_q.put_nowait([motor1_speed*10*(1+speed_up), motor2_speed*8])
        elif(event[0] == 1):
            if(event[1] == 0x127):
                speed_up = event[2]*1.36
                #speed_up = event[2]+1
            #elif(event[1] == 0x126):
